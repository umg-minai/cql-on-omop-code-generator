...
;;; Load OMOP CDM version specified by VERSION, perform adaptation for
;;; MIMIC, then emit FORMAT into the destination specified by TARGET.
(defun do-mimic (version format target)
  (let* ((vanilla-data-model (prepare-model version))
         (data-model (changes-for-mimic vanilla-data-model)))
    (emit data-model format target)
    data-model))

;;; Load OMOP CDM version specified by VERSION, then emit SQL code for
;;; changing the schema of a database from vanilla OMOP CDM to OMOP
;;; CDM with adaptations for MIMIC data.
(defun emit-mimic-schema-changes (version target)
  (let ((data-model (prepare-model version))
        (format     (make-instance 'mimic-schema-changes :schema "cds_cdm")))
    (emit data-model format target)
    data-model))
